...
- **Plugin config exists in the repo at a non-standard path** → use `config_files` to point to it in place. See https://docs.qlty.sh/cli/linter-extensions.
- **Plugin requires extra packages** (parsers, plugins, extensions) → use `extra_packages`. See https://docs.qlty.sh/cli/linter-extensions.
- **No config exists anywhere in the repo** → create one in `.qlty/configs/` and reference it via `config_files`.
- **Plugin cannot be configured without touching files outside `.qlty/`** → comment it out in `qlty.toml` with a note explaining what the user would need to do manually.
...
- **Code smells** → enables Qlty's built-in maintainability analysis. For threshold fields see https://docs.qlty.sh/code-smells.md and https://docs.qlty.sh/complexity.md. "Strict" tightens thresholds ~30%; "relaxed" loosens them
...
- **Extra packages and config files**: follow the decision tree at the top of this skill. See https://docs.qlty.sh/cli/linter-extensions for all options
...
Field reference: https://docs.qlty.sh/cli/qlty-toml.md
