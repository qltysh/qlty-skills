...
**Never copy or duplicate existing config files.** If a plugin's config already exists in the repo at a non-standard location, reference it in place using `config_files` — see https://docs.qlty.sh/cli/linter-extensions for all plugin configuration options (`config_files`, `extra_packages`, `prefix`, `drivers`, etc.). Only create a new config in `.qlty/configs/` if no config exists anywhere in the repo for that plugin. If a plugin cannot be configured without modifying files outside `.qlty/`, comment it out in `qlty.toml` with a note explaining what the user would need to do manually.
